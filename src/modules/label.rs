...
impl Label {
    ...
    pub fn new<T: Into<String>>(text: T, x: f32, y: f32, font_size: u16) -> Self {
        let mut label = Self {
            ...
            y,
            font_size,
            foreground: GREEN, // Default to black
            background: Some(BLACK),  // No background by default
            line_spacing: 1.2,
            font: None,        // Default to None (use system font)
            corner_radius: 0.0, // Default to no rounded corners
            border: true,      // Default to no border
            border_color: RED, // Default border color
            border_thickness: 3.0, // Default border thickness
